...
def main():
    ...
    with open(report_path, "w") as f:
        ...
        f.write("This document summarizes the experiments demonstrating the necessity of core DQN components.\n\n")
    ...
    config_no_target["agent_type"] = "DQN" # Use standard DQN to isolate the target network effect
    run_ablation("No Target Network", config_no_target)
    
    with open(report_path, "a") as f:
        f.write("## 1. Without Target Network\n")
        f.write("- **Method:** Target network updated every step (`target_update_freq = 1`).\n")
    ...
    # It will only train on the absolute most recent transitions, breaking the i.i.d assumption.
    ...
    # We pass only 1 frame instead of 4. The agent is now blind to velocity and direction.
