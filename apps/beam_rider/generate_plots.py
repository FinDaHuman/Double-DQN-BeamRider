...
def main():
    ...
    run_dirs = [os.path.join(logs_base_dir, d)
                for d in os.listdir(logs_base_dir) if "doubledqn" in d]
    if not run_dirs:
    ...
    # Extract Rewards
    steps_reward, rewards = extract_scalars(
        main_run_dir, "Train/Episode_Reward")
