...
def make_video_env(env_name, skip, stack, video_folder):
    ...
    # Re-apply the normal preprocessing wrappers so the agent gets the right
    # inputs
...
def main():
    ...
    print(
        f"Initializing video recording environment for {CONFIG['env_name']}...")
    env = make_video_env(
        CONFIG["env_name"],
        CONFIG["frame_skip"],
        CONFIG["frame_stack"],
    ...
    print(
        f"Recording 3 episodes to find the best attempt (Epsilon: {epsilon})...")
    ...
    env.close()
    print(
        f"Finished recording. The BEST attempt was Episode {best_episode} with Reward: {best_reward}")
    print(
        f"Videos saved to the '{video_dir}/' directory. Use 'rl-video-episode-{best_episode}.mp4' for your presentation.")
