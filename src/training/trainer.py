...
class Trainer:
    ...
    def __init__(self, config: dict):
        ...
        if agent_type == "DoubleDQN":
            self.agent = DoubleDQNAgent(state_shape, num_actions, lr=config["learning_rate"], device=self.device)
        else:
            self.agent = DQNAgent(state_shape, num_actions, lr=config["learning_rate"], device=self.device)
            
        self.replay_buffer = ReplayBuffer(config["replay_capacity"])
        
        os.makedirs("logs", exist_ok=True)
        os.makedirs("models", exist_ok=True)
        self.writer = SummaryWriter(log_dir=f"logs/{agent_type.lower()}_{int(time.time())}")
    ...
    def train(self):
        ...
        print(f"Beginning training for {self.config['total_timesteps']} timesteps.")
        
        for step in tqdm(range(1, self.config["total_timesteps"] + 1), desc="Training"):
            
            # Decay Epsilon
            epsilon = max(
                self.config["epsilon_end"], 
                self.config["epsilon_start"] - step / self.config["epsilon_decay_steps"] * (self.config["epsilon_start"] - self.config["epsilon_end"])
            )
            ...
            if done:
                ...
                self.writer.add_scalar("Train/Episode_Length", episode_length, step)
            ...
            if step > self.config["learning_starts"] and step % self.config["train_frequency"] == 0:
                loss = self.agent.train_step(self.replay_buffer, self.config["batch_size"], self.config["gamma"])
            ...
            if step % 250_000 == 0:
                torch.save(self.agent.online_net.state_dict(), f"models/dqn_step_{step}.pth")
